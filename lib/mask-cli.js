...
if (
  ...
  console.log(
    "Usage: feed-mask <input-json> <output-mask> [--allocation FILE] [--pick rows] [--tab Label:rows] [--summary-file FILE] [--summary TEXT]",
  );
  process.exit(0);
}
...
if (pickSpec) {
  ...
  const allocation = loadAllocation(
    getAllocationPath(document, allocationPath),
  );
  tabs.push(
