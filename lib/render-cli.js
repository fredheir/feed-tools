...
const { getDefaultDocumentPath, getDefaultHtmlPath } = require("./document-paths");
const {
...
function relativizeAssetPaths(document, outputPath) {
  ...
  for (const item of document.items || []) {
    if (item.author) item.author.profile_image_local = toRelative(item.author.profile_image_local);
    for (const media of item.media || []) media.local_src = toRelative(media.local_src);
    for (const card of item.cards || []) card.image_local = toRelative(card.image_local);
  }
}
...
if (
  ...
  console.log(
    "Usage: feed-render [input-json] [output-html] [--pick rows|all] [--tab] [--summary-file FILE] [--summary TEXT] [--no-open]",
  );
  process.exit(0);
}
...
if (!noOpen) openPathOrUrl(outputPath);
