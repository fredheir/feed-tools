...
function normalizeBrowserOptions(options = {}) {
  ...
  return {
    ...
    sessionName: pickStringOption(options.sessionName),
    profile: resolveConfigPath(options.profile),
    statePath: resolveConfigPath(options.statePath || options.state_path),
    headed: options.headed === true && !cdp,
    allowFileAccess: options.allowFileAccess === true,
    colorScheme: pickStringOption(options.colorScheme),
    executablePath: resolveConfigPath(options.executablePath),
    cdp,
    args: toStringList(options.args),
  };
}
