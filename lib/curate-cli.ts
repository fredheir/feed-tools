...
function parseRegex(pattern: string): RegExp | null {
  ...
  if (delimited) {
    const [, expression = "", flags = "i"] = delimited;
    return new RegExp(expression, flags || "i");
  }
  return new RegExp(raw, "i");
}

function parsePatternList(spec: string): RegExp[] {
  return String(spec || "")
    .split(",")
...
function getSearchHaystack(item: FeedItem, category: string): string {
  return [
    item.source || "",
    category || "",
    item.id || "",
...
function summarizeStats(item: FeedItem): string {
...
function buildMatchedRows(
  ...
  allocation: FeedAllocation | null,
  matchers: RegExp[],
  options: { limit?: number | null; fallbackCategory?: string } = {},
): MatchedRow[] {
  const { limit, fallbackCategory = "Other" } = options;
  const rows = buildRows(document)
    .map(({ row, item }: { row: number; item: FeedItem }) => {
...
function printMatchedRows(
  ...
  return rows
    .map(({ row, item, category, hits }: MatchedRow) => {
      return [
        row,
        item.id || "",
        category,
...
function printRenderContext(config: FeedConfig): string {
...
const listing = matchers.length
  ? printMatchedRows(resultDocument, resultAllocation, matchers, { limit })
