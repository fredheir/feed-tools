...
function getAllocationPath(document, explicitPath = null) {
  ...
  const saveDir =
    getCaptureDefaults(config, document.source).save_dir || "/tmp/feed-archive";
  return path.join(saveDir, document.source, "allocation.json");
}
...
function loadAllocationForDocument(document, explicitPath = null) {
  ...
  const allocations = getEnabledSources(config)
    .filter((source) => documentSources.has(source.name))
    .map((source) => {
      const saveDir =
        getCaptureDefaults(config, source.name).save_dir || "/tmp/feed-archive";
      return loadAllocation(path.join(saveDir, source.name, "allocation.json"));
    });
