...
function isSshPrivateKeyFilename(filename: string): boolean {
  return (
    !filename.startsWith(".") &&
...
function checkGitRemote(): CheckResult {
  ...
  if (hasSshAgentKey() || hasSshPrivateKey(sshDir)) {
    return { name: "git-remote", ok: true, detail: redactedRemote };
  }
  return {
    name: "git-remote",
...
function nextActions(
  ...
  if (path === "cdp") return ["Set capture.browser.cdp to the detected CDP endpoint."];
  if (path === "agent-browser") return ["Use the existing agent-browser capture path."];
  if (path === "workspace-chrome") return ["Run ./bin/feed-setup-sandbox, launch workspace Chrome, then rerun doctor."];
