...
const SOURCE_CONFIGS = {
  x: {
    ...
    scrollTopScript: `window.scrollTo({ top: 0, behavior: "instant" })`,
    scrollDownScript: `window.scrollBy({ top: Math.round(window.innerHeight * 0.9), behavior: "instant" })`,
    itemCountExpression: `document.querySelectorAll('article').length`,
    newItemsCheck: (knownCount) =>
      `document.querySelectorAll('article').length > ${knownCount}`,
  ...
  bluesky: {
    ...
    newItemsCheck: (knownCount) =>
      `document.querySelectorAll('[data-testid^="feedItem-by-"]').length > ${knownCount}`,
  ...
  linkedin: {
    ...
    newItemsCheck: (knownCount) =>
      `document.querySelectorAll('[data-id]').length > ${knownCount}`,
