...
function getRenderCss() {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .feed-card { display: grid; grid-template-columns: 72px 1fr; gap: 0; padding: 12px 14px 12px 10px; border-top: 1px solid var(--border); background: linear-gradient(180deg, rgba(255,255,255,0.98), rgba(250,252,253,0.98)); transition: background 140ms ease; }
    ...
    .meta { display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap; }
    .platform-mark { width: 40px; height: 40px; border-radius: 12px; object-fit: cover; box-shadow: 0 8px 20px rgba(110, 125, 140, 0.18); flex: 0 0 auto; align-self: center; }
    .rail-platform-mark { position: relative; z-index: 2; }
    .handle { font-weight: 700; font-size: 14px; }
    .index { color: var(--muted); font-size: 12px; }
    .text { font-size: 16px; line-height: 1.38; white-space: pre-wrap; }
    ...
    @media (max-width: 720px) {
      .feed-card { grid-template-columns: 56px 1fr; padding: 11px 12px 11px 8px; }
      .text { font-size: 14px; }
