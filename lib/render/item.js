...
function getMediaPlatformLabel(sourceName, item = {}) {
  return getPlatformIconMeta(sourceName || item.source).label;
}

function renderMedia(items, sourceName = "") {
  if (!Array.isArray(items) || items.length === 0) return "";
  return items
    .map((item) => {
      if (item.local_video_src) {
        const poster = item.local_src || item.src || "";
        const platformLabel = getMediaPlatformLabel(sourceName, item);
      ...
      const action = item.href
        ? `<span class="media-action">${escapeHtml(item.media_kind === "video" ? `View on ${getMediaPlatformLabel(sourceName, item)}` : "Open media")}</span>`
        : "";
...
function renderItemCard(item, previousItem) {
  ...
  const hasVideo = inlineMedia.some((media) => Boolean(media?.local_video_src));
